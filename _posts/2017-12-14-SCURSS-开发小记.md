...
开始觉得原始的瀑布流模型的开发对变更的需求支持非常有限，增量模型会更适合一些。

同时，对数据库的操作有了更多理解，这一次使用了postgresql是在和mysql, mongodb,等做了很多次的比较之后才决定的，redis也是第一次用到生产环境中，以前只写过小的redis程序，最初配置 postgresql是在一台云服务器上配置的，花了大概一个下午，改了默认的数据库存储位置，自己添加用户名，打开公网监听，尝试备份和备份恢复。

但是完成了此次项目之后，开始体会到docker的优点，现在可以在五分钟内搭建包括postgresql, mysql, mongdb 等数据库并且投入运行，对于其他程序的运行时环境，比如php, python也能在几分钟内就完成项目运行时环境的构建，docker的熟练使用对平时的工作和学习带来了极大的方便。

同时，也体会到了web安全的重要性， 由于使用了redis， 而redis默认是没有密码的，这个问题让我想到了两年前的redis未授权访问，其实现在的版本也存在这个问题，任何人只要知道ip地址便可以进入数据库修改数据，为了连接方便我也没有设置密码，完成项目后我便移除了redis的外网映射，这样只允许容器内的程序调用，对于外界的攻击者就无法进入内部发起攻击了。



- 瀑布流模型是一个很有用的东西，仅限于学生写作业的时候
- 增量模型可能好用一点
- docker真好用, docker-compose 编排容器，非常方便