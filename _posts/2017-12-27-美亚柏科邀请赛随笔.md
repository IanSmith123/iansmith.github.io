...
拿出了1w的字典，php万能比吗字典爆破许久，试过了`root`, `admin`,`administartor`, `webmaster`, `manager`等用户均无法登录，登录的结果给出的回显包括
```
bad request. 
login failed
```
...
```
这个就是一个肉眼可见的`zip`压缩包的十六进制了，于是写入一个二进制文件，解压提示有密码
...
![](http://oqyjccf1n.bkt.clouddn.com/20171227-001626.png)
...
- 大的post包只检查前面几个字节
3. 协议绕过
使用form-data上传而不是www-x-form-data 一般不会检测前者
4. 协议解析不一致
文件名包含换行符， 绕过waf
